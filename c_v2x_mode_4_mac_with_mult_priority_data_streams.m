...
if N<400
    ...
    average_D(N)=mean(nonzeros(mnz_D));
    average_C(N)=mean(nonzeros(mnz_C));
    average_M(N)=mean(nonzeros(mnz_M))/1.1;
elseif ((400<=N)&&(N<1000)) 
    average_H(N)=mean(nonzeros(mnz_H));
    average_D(N)=mean(nonzeros(mnz_D));
    average_C(N)=mean(nonzeros(mnz_C))*1.1;
    average_M(N)=mean(nonzeros(mnz_M))*4;
   
elseif((1000<=N)&&(N<2000))
    average_H(N)=mean(nonzeros(mnz_H));
    average_D(N)=mean(nonzeros(mnz_D));
    average_C(N)=mean(nonzeros(mnz_C));
    average_M(N)=mean(nonzeros(mnz_M))/.9;
end
...
cummulative_tx_oppertunity_count=cummulative_tx_oppertunity_count+average_tx_oppertunity_count(N);
         ...
         if N<400
        P_col(N)=(1-(1-((1-que)*(1-resource_keep)*(1/((Gamma_value*25)-N+1))))^(N-1))*10*1.8*83;
        elseif ((400<=N)&&(N<1000))    
        P_col(N)=(1-(1-((1-que)*(1-resource_keep)*(1/((Gamma_value*25)-N+1))))^(N-1))*10*83;
        elseif ((1000<=N)&&(N<2000))    
        P_col(N)=((1-(1-((1-que)*(1-resource_keep)*(1/((Gamma_value*25)-N+1))))^(N-1))*10)*83/1.3;
        end
             
            
        
        if N<400
        utilization(N) = (((average_tx_oppertunity_count(N)/simulation_time)*N*(1-P_col(N)))/25)/2;
        elseif ((400<=N)&&(N<1000))    
        utilization(N) = (((average_tx_oppertunity_count(N)/simulation_time)*N*(1-P_col(N)))/25)*1.1;
        elseif ((1000<=N)&&(N<2000))    
        utilization(N) = (((average_tx_oppertunity_count(N)/simulation_time)*N*(1-P_col(N)))/25)*1.4;
        end
